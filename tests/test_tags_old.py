...
class MergeDiffTest(TestCase):
    ...
    def test_invalid(self) -> None:
        # This test should check that invalid JSON in block raises an exception
        template = get_template(source=self.block)
        with raises(Exception):  # Should be a JSON parsing error
            template.render(var={"test": "ok"}, block="{test:ok}")  # Invalid JSON
